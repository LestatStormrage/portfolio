...
document.getElementById("footer-copyright-year").textContent = (/* @__PURE__ */ new Date()).getFullYear();
class DynamicAdapt {
  constructor() {
    this.type = "max";
    this.init();
...
function spollers() {
  ...
  if (spollersArray.length > 0) {
    let initOneSpollers2 = function(spollersArray2) {
      const oneSpollers = Array.from(spollersArray2).filter((item) => {
        return item.hasAttribute("data-fls-spollers-one");
      });
      const regular = oneSpollers.filter((item) => {
        return !item.dataset.flsSpollersOne;
      });
      if (regular.length) {
        regular.forEach((item) => item.classList.add("--spoller-one"));
      }
      let mdQueriesArray2 = dataMediaQueries(oneSpollers, "flsSpollersOne");
      if (mdQueriesArray2 && mdQueriesArray2.length) {
        mdQueriesArray2.forEach((mdQueriesItem) => {
          mdQueriesItem.matchMedia.addEventListener("change", function() {
            toggleOne(mdQueriesItem.itemsArray, mdQueriesItem.matchMedia);
          });
          toggleOne(mdQueriesItem.itemsArray, mdQueriesItem.matchMedia);
        });
      }
      function toggleOne(itemsArray, matchMedia) {
        itemsArray.forEach((item) => {
          const el = item.item;
          if (matchMedia.matches) {
            el.classList.add("--spoller-one");
          } else {
            el.classList.remove("--spoller-one");
          }
        });
      }
    }, initSpollers2 = function(spollersArray2, matchMedia = false) {
      ...
      if (el.closest("summary") && el.closest("[data-fls-spollers]")) {
        ...
        if (el.closest("[data-fls-spollers]").classList.contains("--spoller-init")) {
          ...
          const oneSpoller = spollersBlock.classList.contains("--spoller-one");
    ...
    var initOneSpollers = initOneSpollers2, initSpollers = initSpollers2, initSpollerBody = initSpollerBody2, setSpollerAction = setSpollerAction2, hideSpollersBody = hideSpollersBody2;
    initOneSpollers2(spollersArray);
    ...
    async function formSubmitAction(form, e) {
      ...
      if (error === 0) {
        if (form.dataset.flsForm === "ajax") {
          ...
          const contactBody = form.closest(".contact__body");
          contactBody?.classList.add("--sending");
          ...
          if (response.ok) {
            ...
            const contactBody2 = form.closest(".contact__body");
            contactBody2?.classList.remove("--sending");
